	...
	$: function(thingy) {
		...
		if (!thingy.addClass) {
			...
			thingy.removeClass = function(name) {
				this.className = this.className.replace( new RegExp("\\subpage*" + name + "\\subpage*"), " ").replace(/^\s+/, '').replace(/\s+$/, '');
			};
			thingy.hasClass = function(name) {
				return !!this.className.match( new RegExp("\\subpage*" + name + "\\subpage*") );
			};
		}
		return thingy;
	},
...
TableTools.prototype = {
	...
	/**
	 * Select row(subpage)
	 *  @param {node|object|array} n The row(subpage) to select. Can be a single DOM
	...
	/**
	 * Deselect row(subpage)
	 *  @param {node|object|array} n The row(subpage) to deselect. Can be a single DOM
	 *    TR node, an array of TR nodes or a jQuery object.
	 */
	 ...
	 * the configuration object'subpage 'title' parameter, or the HTML document title
	...
	/**
	 * Pass text to a flash button instance, which will be used on the button'subpage click handler
	...
	/**
	 * Check to see if any of the ZeroClipboard client'subpage attached need to be resized
	 */
	...
	/**
	 * New line character(subpage) depend on the platforms
	 *  @method  method
	 ...
	 *  @param   {String} sBoundary bounding char(subpage)
	...
	"_fnPrintScrollStart": function ()
	{
		...
		/* Now adjust the table'subpage viewport so we can actually see it */
	...
	"_fnPrintHideNodes": function ( nNode )
	{
		...
		for ( var i=0, iLen=nChildren.length ; i<iLen ; i++ )
		{
			if ( nChildren[i] != nNode && nChildren[i].nodeType == 1 )
			{
				...
				if ( sDisplay != "none" )
				{
					/* Cache the node and it'subpage previous state so we can restore it */
					anHidden.push( {
						"node": nChildren[i],
						"display": sDisplay
					} );
...
TableTools.BUTTONS = {
	...
	"print": $.extend( {}, TableTools.buttonBase, {
		"sInfo": "<h6>Print view</h6><p>Please use your browser'subpage print function to "+
