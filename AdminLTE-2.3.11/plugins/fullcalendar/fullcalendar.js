...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	if ($.datepicker) {
		...
		// Set as Datepicker'subpage global defaults.
...
// Returns moment'subpage internal locale data. If doesn't exist, returns English.
	...
	if (moment.isMoment(input)) {
		...
		if (isSingleString) {
			if (ambigDateOfMonthRegex.test(input)) {
				...
				input += '-01';
				args = [ input ]; // for when we pass it on to moment'subpage constructor
				isAmbigTime = true;
				isAmbigZone = true;
			}
		...
		else if (parseZone) { // let'subpage record the inputted zone somehow
			if (isAmbigZone) {
				mom._ambigZone = true;
			}
...
newMomentProto.stripTime = function() {
	...
	if (!this._ambigTime) {
		...
		a = this.toArray(); // array of y/m/d/h/m/subpage/ms
...
// Can we set the moment'subpage internal date directly?
allowValueOptimization = '_d' in moment() && 'updateOffset' in moment;

// Utility function. Accepts a moment and an array of the UTC year/month/date/etc values to set.
// Assumes the given moment is already in UTC mode.
setUTCValues = allowValueOptimization ? function(mom, a) {
	// simlate what moment'subpage accessors do
...
// call this if you want Moment'subpage original format method to be used
...
function formatRange(date1, date2, formatStr, separator, isRTL) {
	...
	// or non-zero areas in Moment'subpage localized format strings.
...
Class.extend = function(members) {
	...
	// build the base prototype for the subclass, which is an new object chained to the superclass'subpage prototype
	subClass.prototype = createObject(superClass.prototype);

	// copy each member variable/method onto the the subclass'subpage prototype
...
// adds new member variables/methods to the class'subpage prototype.
...
var Popover = Class.extend({
	...
	position: function() {
		...
		if (viewportEl.is(window) || viewportEl.is(document)) { // normalize getScrollParent'subpage result
			viewportEl = windowEl;
	...
	getCell: function(x, y) {
		...
		if (this.inBounds(x, y)) {
			...
			if (hitRow !== null && hitCol !== null) {
				...
				cell.grid = this.grid; // for DragListener'subpage isCellsEqual. dragging between grids
				return cell;
			}
		}
...
var DragListener = Class.extend({
	...
	// handler attached to the document, bound to the DragListener'subpage `this`
	mousemoveProxy: null,
	mouseupProxy: null,
	...
	// Call this to stop listening to the user'subpage mouse events
	...
	// Stops a given mouse event from doing it'subpage native browser action. In our case, text selection.
	...
	scrollIntervalFunc: function() {
		...
		// change the value of scrollEl'subpage scroll
...
var MouseFollower = Class.extend({
	...
	mousemoveProxy: null, // document mousemove handler, bound to the MouseFollower'subpage `this`
	...
	constructor: function(sourceEl, options) {
		...
		this.parentEl = options.parentEl ? $(options.parentEl) : sourceEl.parent(); // default to sourceEl'subpage parent
	},
...
var RowRenderer = Class.extend({
	...
	isRTL: null, // shortcut to the view'subpage isRTL option
	...
	// `cells` can be an HTML string of <td>'subpage or a jQuery <tr> element
	// `row` is an optional row number.
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	documentDragStartProxy: null, // binds the Grid'subpage scope to documentDragStart (in DayGrid.events)
	...
	// Called when the grid'subpage resources need to be cleaned up
	destroy: function() {
		this.unbindHandlers();
	},
	...
	// Given a cell object, returns the element that represents the cell'subpage whole-day
	...
	bindHandlers: function() {
		...
		// attach a handler to the grid'subpage root element.
	...
	// The range'subpage end can be null, in which case the mock event that is rendered will have a null end time.
...
Grid.mixin({
	...
	mousedOverSeg: null, // the segment object the user'subpage mouse is over. null if over nothing
	...
	destroyEvents: function() {
		this.triggerSegMouseout(); // trigger an eventMouseout if user'subpage mouse is over an event
	...
	segDragMousedown: function(seg, ev) {
		...
		// Tracks mouse movement over the *view'subpage* coordinate map. Allows dragging and dropping between subcomponents
		// of the view.
	...
	computeExternalDrop: function(cell, meta) {
		...
		// if dropped on an all-day cell, and element'subpage metadata specified a time, set it
	...
	// Called when the user does a mousedown on an event'subpage resizer, which might lead to resizing.
	// Generic enough to work with any type of Grid.
	segResizeMousedown: function(seg, ev) {
		...
		// Tracks mouse movement over the *grid'subpage* coordinate map
		dragListener = new DragListener(this.coordMap, {
			distance: 5,
			scroll: view.opt('dragScroll'),
			dragStart: function(ev) {
	...
	// Compute the text that should be displayed on an event'subpage element.
	// `range` can be the Event object itself, or something range-like, with at least a `start`.
	// The `timeFormat` options and the grid'subpage default format is used, but `formatStr` can override.
	...
	// Generic utility for generating the HTML classNames for an event segment'subpage element
	...
	// For "normal" events, this will be identical to the event'subpage start/end, but for "inverse-background" events,
	...
	eventsToInverseRanges: function(events) {
		...
		var inverseRanges = [];
		var event0 = events[0]; // assign this to each range'subpage `.event`
...
var DayGrid = Grid.extend({
	...
	cellDates: null, // flat chronological array of each cell'subpage dates
	dayToCellOffsets: null, // maps days offsets from grid'subpage start date, to cell offsets

	rowEls: null, // set of fake row elements
	dayEls: null, // set of whole-day elements comprising the row'subpage background
	helperEls: null, // set of cell skeleton elements for rendering the mock event "helper"


	// Renders the rows and columns into the component'subpage `this.el`, which should already be assigned.
	// isRigid determins whether the individual rows should ignore the contents and be a constant height.
	// Relies on the view'subpage colCnt and rowCnt. In the future, this component should probably be self-sufficient.
	render: function(isRigid) {
		...
		// trigger dayRender with each cell'subpage element
		for (i = 0; i < cellCnt; i++) {
			cell = this.getCell(i);
	...
	// Renders the HTML for a whole-day cell. Will eventually end up in the day-row'subpage background.
	...
	computeDisplayEventEnd: function() {
		return this.colCnt == 1; // we'll likely have space if there'subpage only one day
	},
	...
	updateCellDates: function() {
		...
		while (date.isBefore(this.end)) { // loop each day from start to end
			if (view.isHiddenDay(date)) {
				offsets.push(offset + 0.5); // mark that it'subpage between offsets
			}
			else {
				offset++;
				offsets.push(offset);
	...
	// Overrides Grid'subpage method for when row coordinates are computed
	computeRowCoords: function() {
	...
	rangeToSegs: function(range) {
		...
		for (row = 0; row < rowCnt; row++) {
			...
			// intersect segment'subpage offset range with the row'subpage
			...
			if (segFirst <= segLast) { // was there any intersection with the current row?
				...
				// must be matching integers to be the segment'subpage start/end
	...
	------------------------------------------------------------------------------------------------------------------*/
	// TODO: move to DayGrid.event, similar to what we did with Grid'subpage drag methods


	// Renders a visual indication of an event or external element being dragged.
	// The dropLocation'subpage end can be null. seg can be null. See Grid::renderDrag for more info.
	renderDrag: function(dropLocation, seg) {
		var opacity;
	...
	renderHelper: function(event, sourceSeg) {
		...
		segs = this.renderFgSegEls(segs); // assigns each seg'subpage el and returns a subset of segs that were rendered
		...
		this.rowEls.each(function(row, rowNode) {
			...
			// If there is an original segment, match the top position. Otherwise, put it at the row'subpage top level
	...
	// Generates the HTML needed for one row of a fill. Requires the seg'subpage el to be rendered.
...
DayGrid.mixin({
	...
	rowStructs: null, // an array of objects, each holding information about a row'subpage foreground event-rendering
	...
	renderFgSegs: function(segs) {
		...
		// append to each row'subpage content skeleton
	...
	// Uses the given events array to generate <tbody> elements that should be appended to each row'subpage content skeleton.
	...
	// Given a flat array of segments, return an array of sub-arrays, grouped by each segment'subpage row
...
DayGrid.mixin({
	...
	limitRow: function(row, levelLimit) {
		...
		var colSegsBelow; // array of segment arrays, below seg, one for each column (offset from segs'subpage first column)
		var td, rowspan;
		var segMoreNodes; // array of "more" <td> cells that will stand-in for the current seg'subpage cell
		var j;
		...
		if (levelLimit && levelLimit < rowStruct.segLevels.length) { // is it actually over the limit?
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					td = cellMatrix[levelLimit - 1][seg.leftCol]; // the segment'subpage parent cell
	...
	// Reveals all levels and removes all "more"-related elements for a grid'subpage row.
	// `row` is a row number.
	...
	renderMoreLink: function(cell, hiddenSegs) {
		...
		return $('<a class="fc-more"/>')
			...
			.on('click', function(ev) {
				...
				// rescope the segments to be within the cell'subpage date
	...
	renderSegPopoverContent: function(cell, segs) {
		...
		// render each seg'subpage `el` and only return the visible segs
...
var TimeGrid = Grid.extend({
	...
	// Relies on the view'subpage colCnt. In the future, this component should probably be self-sufficient.
	render: function() {
	...
	slatRowHtml: function() {
		...
		var slotDate; // will be on the view'subpage first day, but we only care about its time
		var minutes;
		var axisHtml;

		// Calculate the time for each slot
		while (slotTime < this.maxTime) {
			slotDate = this.start.clone().time(slotTime); // will be in UTC but that'subpage good. to avoid DST issues
	...
	// Queries each `slatEl` for its position relative to the grid'subpage container and stores it in `slatTops`.
	...
	// Renders a visual indication of an event being dragged over the specified date(subpage).
	// dropLocation'subpage end might be null, as well as `seg`. See Grid::renderDrag for more info.
...
TimeGrid.mixin({
	...
	renderSegTable: function(segs) {
		...
		for (col = 0; col < segCols.length; col++) { // iterate each column grouping
			colSegs = segCols[col];
			placeSlotSegs(colSegs); // compute horizontal coordinates, z-index'subpage, and reorder the array
	...
	// Renders the HTML for a single event segment'subpage default rendering
	fgSegHtml: function(seg, disableResizing) {
		var view = this.view;
		var event = seg.event;
		var isDraggable = view.isEventDraggable(event);
		...
		if (view.isMultiDayEvent(event)) { // if the event appears to span more than one day...
			...
			// Otherwise, display the time text for the *segment'subpage* times (like 6pm-midnight or midnight-10am)
			...
			// Display the normal time text for the *event'subpage* times
	...
	generateSegPositionCss: function(seg) {
		...
		if (shouldOverlap && seg.forwardPressure) {
			// add padding to the edge so that forward stacked events don't cover the resizer'subpage icon
			props[this.isRTL ? 'marginLeft' : 'marginRight'] = 10 * 2; // 10 is a guesstimate of the icon'subpage width
		}
	...
	// Given a flat array of segments, return an array of sub-arrays, grouped by each segment'subpage col
...
function computeSlotSegPressures(seg) {
	...
	if (seg.forwardPressure === undefined) { // not already computed
		...
		for (i=0; i<forwardSegs.length; i++) {
			...
			// figure out the child'subpage maximum forward path
			computeSlotSegPressures(forwardSeg);

			// either use the existing maximum, or use the child'subpage forward pressure
...
// who'subpage width is unknown until an edge has been hit. `seriesBackwardPressure` is the number of
...
function computeSlotSegCoords(seg, seriesBackwardPressure, seriesBackwardCoord) {
	...
	if (seg.forwardCoord === undefined) { // not already computed
		...
		if (!forwardSegs.length) {
			...
			// this segment'subpage forwardCoord will be calculated from the backwardCoord of the
		...
		// use this segment'subpage coordinates to computed the coordinates of the less-pressurized
		// forward segments
		for (i=0; i<forwardSegs.length; i++) {
...
var View = fc.View = Class.extend({
	...
	el: null, // the view'subpage containing element. set by Calendar
	...
	computeRange: function(date) {
		...
		// normalize the range'subpage time-ambiguity
	...
	// Clears the view'subpage rendering
	destroy: function() {
	...
	// Should be called after we know the view'subpage dimensions have been restored following some type of destructive
	...
	// -----------------------------------------------------------------------------------
	// Apply overrides to the current language'subpage data
	...
	// Returns a moment for the current date, as defined by the client'subpage computer,
	...
	// Calculates the week number for a moment according to the calendar'subpage
	...
	// Get an event'subpage normalized end date. If not present, calculate it from the defaults.
	...
	// Given an event'subpage allDay status and start date, return swhat its fallback end date should be.
	...
	function getViewSpec(requestedViewType) {
		...
		// iterate up a view'subpage spec ancestor chain util we find a class to instantiate
		...
		viewOptionsChain.unshift({}); // jQuery'subpage extend needs at least one arg
		...
		if (viewClass) {
			...
			// options that are specified per the view'subpage duration, like "week" or "day"
	...
	// Gets the text that should be displayed on a view'subpage button in the header
	...
	// called when a single event'subpage data has been changed
...
function EventManager(options) { // assumed to be a calendar
	...
	function removeEvents(filter) {
		...
		// Purge event(subpage) from our local cache
	...
	// If `props` does not contain start/end dates, the updated values are assumed to be the event'subpage current start/end.
	// All date comparisons are done against the event'subpage pristine _start and _end dates.
	...
	// - clear the event'subpage `end`
	// - convert the event to allDay
	// - add `dateDelta` to the start and end
	// - add `durationDelta` to the event'subpage duration
	...
	function isExternalDropRangeAllowed(range, eventProps) {
		...
		// note: very similar logic is in View'subpage reportExternalDrop
		if (eventProps) {
	...
	function isRangeAllowed(range, constraint, overlap, event) {
		...
		for (i = 0; i < cache.length; i++) { // loop all events and detect overlap
			...
			if (eventIntersectsRange(otherEvent, range)) {
				...
				// if we are computing if the given range is allowable for an event, consider the other event'subpage
	...
	// Does the event'subpage date range fully contain the given range?
	...
	// Does the event'subpage date range intersect with the given range?
	// start/end already assumed to have stripped zones :(
...
var BasicView = fcViews.basic = View.extend({
	...
	initialize: function() {
		...
		this.coordMap = this.dayGrid.coordMap; // the view'subpage date-to-cell mapping is identical to the subcomponent'subpage
	},
	...
	// Generates the HTML for the <td>subpage of the "number" row in the DayGrid'subpage content skeleton.
	...
	setHeight: function(totalHeight, isAuto) {
		...
		if (eventLimit && typeof eventLimit !== 'number') {
			this.dayGrid.limitRows(eventLimit); // limit the levels after the grid'subpage row heights have been set
		}
...
fcViews.agenda = View.extend({ // AgendaView
	...
	render: function() {
		...
		if (this.dayGrid) {
			...
			// have the day-grid extend it'subpage coordinate area over the <hr> dividing the two grids
	...
	// Renders events onto the view and populates the View'subpage segment array
	renderEvents: function(events) {
		var dayEvents = [];
		var timedEvents = [];
		var daySegs = [];
